...
{
    int val;
    pthread_mutex_lock(&push_mutex);        // push is locked
    sleep(2);                               // wait 2 seconds
    pthread_mutex_lock(&pop_mutex);         // pop is locked
    printf("Enter val to push: ");
    scanf("%d", &val);
    top++;
    stack[top] = val;
    pthread_mutex_unlock(&pop_mutex);       // pop unlocked
    pthread_mutex_unlock(&push_mutex);      // push unlocked
...
{
    int k;
    pthread_mutex_lock(&push_mutex);        // attempt to lock push, but first thread locks it so it has to wait
    sleep(5);
